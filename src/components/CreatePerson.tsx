...
const createPerson = async ({
  ...
  name,
  age,
}: ICreatePersonParams): Promise<IPerson> => {
...
const CreatePerson = () => {
  ...
  // https://cangsdarm.github.io/react-query-web-i18n/guides&concepts/mutations/
  ...
  return (
    <>
      {mutation.isLoading ? (
        ...
        <p>发生错误:{mutation?.error?.message}</p>
      ) : null}
      {mutation.isSuccess ? (
        <p>
          添加person成功！name:{mutation?.data?.name} age:{mutation?.data?.age}
        </p>
      ) : null}
